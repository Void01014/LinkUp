...
on([
    'echo-private:chat.' . Auth::id() . ',.MessageSent' => function ($event) {
        dd($event);
        ...
        logger('message from me:' . $messageModel->content);
    },
    ...
    @foreach ($messages as $message)
        ...
        <div class="flex {{ $isMe ? 'justify-end' : 'justify-start' }}">
            <div class="max-w-xs lg:max-w-md">
                <div class="px-4 py-2.5 shadow-lg rounded-2xl {{ $isMe 
                ...
                <div class="flex items-center gap-1 mt-1 px-2 {{ $isMe ? 'justify-end' : 'justify-start' }}">
                    ...
                    @if ($isMe)
                        <svg class="w-4 h-4 {{ $message->is_read ? 'text-blue-500' : 'text-gray-400' }}" fill="currentColor" viewBox="0 0 20 20">
